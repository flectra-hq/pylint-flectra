...
class WrapperModuleChecker(BaseChecker):
    ...
    msg_name_key = None
    flectra_node = None
    flectra_module_name = None
    manifest_file = None
    module = None
    manifest_dict = None
    is_main_flectra_module = None
    ...
    def wrapper_visit_module(self, node):
        ...
        if manifest_file:
            ...
            self.flectra_node = node
            self.flectra_module_name = os.path.basename(
                os.path.dirname(self.flectra_node.file))
            with open(self.manifest_file) as f_manifest:
                self.manifest_dict = ast.literal_eval(f_manifest.read())
        elif self.flectra_node and not os.path.dirname(self.flectra_node.file) in \
                os.path.dirname(node.file):
            # It's not a sub-module python of a flectra module and
            #  it's not a flectra module
            self.flectra_node = None
            self.flectra_module_name = None
            self.manifest_dict = None
            self.manifest_file = None
        self.is_main_flectra_module = False
        if self.flectra_node and self.flectra_node.file == node.file:
            self.is_main_flectra_module = True
        self.node = node
        ...
        for msg_code, (title, name_key, description) in \
            ...
            check_method = getattr(
                ...
                None)
            is_flectra_check = self.is_main_flectra_module and \
                msg_code[1:3] == str(settings.BASE_OMODULE_ID)
            is_py_check = msg_code[1:3] == str(settings.BASE_PYMODULE_ID)
            if callable(check_method) and (is_flectra_check or is_py_check):
                if not check_method():
    ...
    def set_extra_file(self, node, msg_args, msg_code):
        ...
        if not fmatch or not msg.startswith(r"%s"):
            return msg_args
        module_path = os.path.dirname(self.flectra_node.file)
    ...
    def filter_files_ext(self, fext, relpath=True, skip_examples=True):
        """Filter files of flectra modules with a file extension.
    ...
    def _skip_files_ext(self, fext, fnames):
        ...
        Eg: '<!-- pylint-flectra:disable=deprecated-data-xml-node -->'
        inside the xml resource"""
        if fext != '.xml':
            return fnames
        info_called = [item[3] for item in inspect.stack() if
                       'modules_flectra' in item[1] and
    ...
    def get_xml_records(self, xml_file, model=None, more=None):
        ...
        return doc.xpath("/openerp//record" + model_filter + more_filter) + \
            doc.xpath("/flectra//record" + model_filter + more_filter) \

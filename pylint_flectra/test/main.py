...
from pylint_flectra import misc
...
EXPECTED_ERRORS = {
    ...
    'flectra-addons-relative-import': 4,
...
class MainTest(unittest.TestCase):
    def setUp(self):
        self.default_options = [
            '--load-plugins=pylint_flectra', '--reports=no', '--msg-template='
        ...
        self.default_extra_params = [
            '--disable=all',
            '--enable=flectralint,pointless-statement,trailing-newlines',
        ]
    ...
    def tearDown(self):
        ...
        prefix = os.path.expanduser(os.environ.get('PYLINT_FLECTRA_STATS',
                                    '~/pylint_flectra_cprofile'))
    ...
    def test_25_checks_without_coverage(self):
        """All flectralint errors vs found"""
        # Some messages can be excluded as they are only applied on certain
        # Flectra versions (not necessarily 1.0).
        excluded_msgs = {
            'xml-deprecated-qweb-directive',
        }
        extra_params = ['--valid_flectra_versions=1.0']
    ...
    def test_90_valid_flectra_versions(self):
        """Test --valid_flectra_versions parameter when is '1.0'"""
        extra_params = ['--valid_flectra_versions=1.0',
                        '--disable=all',

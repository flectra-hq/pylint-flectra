"""Visit module to add flectra checks
"""
...
FLECTRA_MSGS = {
    ...
    # Visit flectra module with settings.BASE_OMODULE_ID
    ...
    'W%d50' % settings.BASE_OMODULE_ID: (
        'Same Flectra module absolute import. You should use '
        'relative import with "." '
        'instead of "openerp.addons.%s"',
        'flectra-addons-relative-import',
        settings.DESC_DFLT
    ),
    ...
    'W%d39' % settings.BASE_OMODULE_ID: (
        '%s Use <flectra> instead of <flectra><data> or use <flectra noupdate="1">'
        'instead of <flectra><data noupdate="1">',
...
DFLT_IMPORT_NAME_WHITELIST = [
    # self-flectra
    'flectra', 'openerp',
    # Known external packages of flectra
...
class ModuleChecker(misc.WrapperModuleChecker):
    ...
    options = (
        ...
        ('import_name_whitelist', {
            ...
            'help': 'List of known import dependencies of flectra,'
            ' separated by a comma.'
        }),
        ('jslintrc', {
            'type': 'string',
            'metavar': '<path to file>',
            'default': os.environ.get('PYLINT_FLECTRA_JSLINTRC') or DFTL_JSLINTRC,
            'help': ('A path to a file that contains a configuration file of '
                     'javascript lint. You can use the environment variable '
                     '"PYLINT_FLECTRA_JSLINTRC" too. Default: %s' % DFTL_JSLINTRC)
        }),
    ...
    def visit_assign(self, node):
        ...
        if node_left.name == '_name':
            node.parent.flectra_attribute_name = node.value.value
            return
        _name = getattr(node.parent, 'flectra_attribute_name', None)
        _inherit = node.value.value
        if _name and _name != _inherit:
            # Skip _name='model.name' _inherit='other.model' because is valid
            return
        key = (self.flectra_node, _inherit)
    ...
    def close(self):
        ...
        for (flectra_node, class_dup_name), nodes in self.inh_dup.items():
            if len(nodes) == 1:
                continue
            path_nodes = []
            for node in nodes[1:]:
                relpath = os.path.relpath(node.file,
                                          os.path.dirname(flectra_node.file))
    ...
    def _get_flectra_module_imported(self, node):
        flectra_module = []
        if isinstance(node, astroid.ImportFrom) and \
                ('openerp.addons' in node.modname or
                 'flectra.addons' in node.modname):
            packages = node.modname.split('.')
            if len(packages) >= 3:
                # from openerp.addons.flectra_module import models
                flectra_module.append(packages[2])
            else:
                # from openerp.addons import flectra_module
                flectra_module.append(node.names[0][0])
        elif isinstance(node, astroid.Import):
            for name, _ in node.names:
                if 'openerp.addons' not in name and 'flectra.addons' not in name:
                    continue
                packages = name.split('.')
                if len(packages) >= 3:
                    # import openerp.addons.flectra_module
                    flectra_module.append(packages[2])
        return flectra_module

    def check_flectra_relative_import(self, node):
        if self.flectra_module_name in self._get_flectra_module_imported(node):
            self.add_message('flectra-addons-relative-import', node=node,
                             args=(self.flectra_module_name))
    ...
    def _check_imported_packages(self, node, module_name):
        ...
        if not self.manifest_dict:
            # skip if is not a module of flectra
            return
    ...
    @utils.check_messages('flectra-addons-relative-import',
                          'missing-import-error',
                          'missing-manifest-dependency')
    def visit_importfrom(self, node):
        self.check_flectra_relative_import(node)
    ...
    def _check_duplicate_id_csv(self):
        """Check duplicate xml id in ir.model.access.csv files of a flectra module.
        :return: False if exists errors and
                 add list of errors in self.msg_args
        """
        all_csv_ids = []
    ...
    def _check_duplicate_xml_fields(self):
        """Check duplicate field in all record of xml files of a flectra module.
    ...
    def _check_deprecated_data_xml_node(self):
        """Check deprecated <data> xml node inside <flectra> xml node
        :return: False if found <data> xml node inside <flectra> xml node"""
        xml_files = self.filter_files_ext('xml')
        self.msg_args = []
        for xml_file in xml_files:
            doc = self.parse_xml(os.path.join(self.module_path, xml_file))
            flectra_nodes = doc.xpath("/flectra") \
                if not isinstance(doc, string_types) else []
            children, data_node = ((flectra_nodes[0].getchildren(),
                                    flectra_nodes[0].findall('data'))
                                   if flectra_nodes else ([], []))
            if len(children) == 1 and len(data_node) == 1:
                lineno = flectra_nodes[0].sourceline
    ...
    def _check_xml_attribute_translatable(self):
        ...
        """
        if (self.linter._all_options['valid_flectra_versions'].config
                .valid_flectra_versions != ['1.0']):
            return True
        self.msg_args = []
        for xml_file in self.filter_files_ext('xml', relpath=True):
            for record in self.get_xml_records(
    ...
    def _check_xml_deprecated_tree_attribute(self):
        ...
        valid_versions = set(
            self.linter._all_options['valid_flectra_versions'].config
            .valid_flectra_versions)
    ...
    def _check_xml_deprecated_qweb_directive(self):
        ...
        valid_versions = set(self.linter._all_options[
            'valid_flectra_versions'].config.valid_flectra_versions)
        if not valid_versions & {'1.0'}:
            return True

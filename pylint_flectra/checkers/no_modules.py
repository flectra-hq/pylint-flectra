...
from .. import misc
from .modules_flectra import DFTL_MANIFEST_DATA_KEYS
...
DFTL_VALID_FLECTRA_VERSIONS = [
    '1.0',
]
DFTL_MANIFEST_VERSION_FORMAT = r"({valid_flectra_versions})\.\d+\.\d+\.\d+$"
DFTL_CURSOR_EXPR = [
    'self.env.cr', 'self._cr',  # new api
    'self.cr',  # controllers and test
    'cr',  # old api
]
DFTL_FLECTRA_EXCEPTIONS = [
...
DFTL_DEPRECATED_FIELD_PARAMETERS = [
    # From flectra/flectra 10.0: flectra/flectra/fields.py:29
...
class NoModuleChecker(BaseChecker):
    ...
    options = (
        ...
        ('manifest_version_format', {
            ...
            'Use "{valid_flectra_versions}" to check the parameter of '
        ...
        ('flectra_exceptions', {
            'type': 'csv',
            'metavar': '<comma separated values>',
            'default': DFTL_FLECTRA_EXCEPTIONS,
            'help': 'List of flectra exceptions separated by a comma.'
        }),
        ('valid_flectra_versions', {
            'type': 'csv',
            'metavar': '<comma separated values>',
            'default': DFTL_VALID_FLECTRA_VERSIONS,
            'help': 'List of valid flectra versions separated by a comma.'
        }),
        ('no_missing_return', {
            'type': 'csv',
            'metavar': '<comma separated values>',
    ...
    def formatversion(self, string):
        valid_flectra_versions = '|'.join(
            map(re.escape, self.config.valid_flectra_versions))
        self.config.manifest_version_format_parsed = (
            self.config.manifest_version_format.format(
                valid_flectra_versions=valid_flectra_versions))
    ...
    def visit_raise(self, node):
        ...
        if isinstance(argument, astroid.Const) and \
                ...
                func_name in self.config.flectra_exceptions:
            self.add_message(
